...
public class movieCut {
	...
	public static void main(String[] args) throws IOException {
		...
		removeTime = properties.getProperty("removeTime");
		removeBlockTime = Double.valueOf(properties.getProperty("removeBlockTime"));

		// カウンターのパスを指定
		String counterPath = currentPath + "\\input_counter";
		File counterFolder = new File(counterPath);
		File[] files = counterFolder.listFiles();
		String vCounterPath = "";
		for (File file : files) {
			if (file.isFile()) {
				vCounterPath = file.getName();
			}
		}
		...
		VideoCapture vc = new VideoCapture(counterFolder + "\\" + vCounterPath);
		...
		while (frameCnt < totalFrames - 1) {
			...
			if (frameCnt % frameSkipCnt == 3) {
				...
				if (prevGrayFrame != null) {
					...
					String console = "start=" + String.format("%02d:%02d:%02d", (int) (tmpFrame/ frameRate / 3600), (int) ((tmpFrame/ frameRate % 3600) / 60), (int) (tmpFrame/ frameRate % 60)) + " " +
							"end=" + String.format("%02d:%02d:%02d", (int) (frameCnt/ frameRate / 3600), (int) ((frameCnt/ frameRate % 3600) / 60), (int) (frameCnt/ frameRate % 60)) + " totalDiff:" + totalDiff;
...
//				diff = false;
		...
		if(counterFlg ==1) {
			for(List<Integer> itemList : frameResultList) {
				...
				// FFmpegコマンドを構築
				String command = currentPath + "\\ffmpeg.exe -ss " + startCommand + " -to " + endCommand + " -i " + counterFolder + "\\" +  vCounterPath + " -c:v copy -c:a aac " + currentPath + "\\trim_counter" + "\\c("+ ++count + ")" + startName + "_" + endName + ".mp4\n";
				System.out.println(command);
				fw.write(command);
			}
		}
